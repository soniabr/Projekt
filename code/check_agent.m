...
for d=1:12                                  % districts
    ...
    agent(i).points = agent(i).points + (1-(agent(i).origin-Avg(1)))*tol(1); % origin
    agent(i).points = agent(i).points + (1-abs(agent(i).age-Avg(2))/30)*tol(2); % age
    agent(i).points = agent(i).points + (1-(agent(i).famstat-Avg(3)))*tol(3); % family situation
    ...
    for nb=1:4    
        ...
        agent(i).points = agent(i).points + (agent(i).famstat==agent(n(nb)).famstat)/4*tol(3);
        agent(i).points = agent(i).points + (agent(i).origin==agent(n(nb)).origin)/4*tol(3);
        agent(i).points = agent(i).points + (round(agent(i).age/10)==round(agent(n(nb)).age/10))/4*tol(3);
